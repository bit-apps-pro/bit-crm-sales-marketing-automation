...
export default function useSaveImap(form: FormInstance) {
  ...
  const { data, error, isError, isPending, mutateAsync } = useMutation<
    ...
    ImapType
  >({
    mutationFn: (data: ImapType) => queryRequest<ImapType>('imaps/store', data),
    mutationKey: ['imap', 'store'],
    onError: error => {
  ...
  return {
    imap: data,
    imapStore: mutateAsync,
    isStoringImap: isPending
  }
}
