...
export default function useEmptyTrash() {
  ...
  const { isPending, mutateAsync } = useMutation<Response<string>, Response<string>>({
    mutationFn: () => queryRequest('trashes/empty'),
    mutationKey: ['trashes', 'empty'],
    onError: error => {
